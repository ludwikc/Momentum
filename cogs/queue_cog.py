...
class QueueCog(commands.Cog):
    ...
    async def advance_queue(self, channel):
        """Advance the queue to the next speaker. Used by both slash commands and voice events."""
        self.current_speaker_id = None
        self.queue.pop(0)

        if self.queue:
            next_speaker = self.queue[0][0]
            await channel.send(f"{next_speaker.mention}, Twoja kolej!")
            await channel.send(self.format_queue())
        else:
            await channel.send("No i kolejka opustoszała.")
            self.counter = 0
    ...
    async def kolejka_slash(self, interaction: discord.Interaction):
        ...
        if any(entry[0].id == interaction.user.id for entry in self.queue):
            await interaction.response.send_message(f"{interaction.user.mention}, już jesteś w kolejce! 😉")
            return
    ...
    async def kolejka_next_slash(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Przesuwam kolejkę.")
        await self.advance_queue(target_channel)
    ...
    async def kolejka_done_slash(self, interaction: discord.Interaction):
        ...
        is_first_in_queue = self.queue[0][0].id == interaction.user.id

        if not (is_speaker or is_first_in_queue):
            await interaction.response.send_message(f"{interaction.user.mention}, to nie Twoja kolej! 😉")
            return

        target_channel = self.get_target_channel() or interaction.channel
        await interaction.response.send_message("Zakończyłeś swoją kolej.")
        await self.advance_queue(target_channel)
