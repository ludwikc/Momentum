...
class leaderboard(commands.Cog):
    ...
    async def leaderboard(self, ctx, activity: discord.Option = dropdown_box):
        ...
        ).sort(f"streaks.activity", -1)
        sorted_leaderboard = sorted(
            leaderboard, key=lambda x: x["streaks"][activity], reverse=True
        )[:10]
        emoji = act[activity]

        embed = Embed(
            title=f"🏆 Leaderboard dla {activity.capitalize()} {emoji}", color=0x280586
        )
